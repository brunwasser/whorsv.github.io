...
* **se_logor**: Continuous variable representing the standard error of the log~e~OR
...
* **minanyneof**: Binary factor indicating whether or not the effect size estimate was based on an analysis that limited potential confounding due to differences in the exposed and comparator groups in markers of neonatal health (e.g., Apgar scores)
...
* **Design**: 3-level factor distinguishing between the 3 primary study design types: *RSV Medical Event*, *RSV Surveillance* studies, and *RSV Prophylaxis* studies
...
* **x~6~^ij^** = confounding due to second-hand smoke exposure either prenatally or in early life: 0=no attempt to limit potential confounding due to smoking exposure; 1= confounding limited at least somewhat
...
**Model 1.1:** We run the *a priori* model for *RSV LRTI Exposure* studies. First, we set the reference level for all effect modifiers to the modal value. Consequently, the intercept represents the conditional mean effect size estimate when all effect modifiers are held at their most common value. 
```{r }
...
**Model 1.2:** We rerun Model 1.1 after collapsing the *school age* and *adolescence/adulthood* categories for the *outcomeagecat2f* variable. It is now a binary variable called *preschool*. This effect modifier now compares effect size esitmates measured during the preschool years to estimates measured outside of the preschool years: preschool vs. schoolage/adolescence/adulthood. This may fix the *df* problem. Additionally, we dropped the two interaction terms as there was no evidence that they were significant and they had relatively small *df*.
...
There is no evidence that limiting potential confounding due to early life 2nd-hand smoke exposure modifies the mean effect size for the effect of RSV LRTI exposure on subsequent wheezing illness. Further, adding this effect modifier does not alter our conclusions about any of the other primary effect modifiers of interest.
