...
# Overview
This page presents a step-by-step summary of the analytic procedures for the paper: **Assessing causal inference of respiratory syncytial virus lower respiratory tract infection on subsequent wheezing illness using meta analytic strategies**. The R code for all output can be accessed by clicking on the corresponding "Code" buttons. The analyses can be reproduced within the R environment using these codes and the required packages (see section 3 below). The primary analytic models were Robust Variance Estimation (RVE) meta-regressions [(Hedges et al. 2010)](https://www.ncbi.nlm.nih.gov/pubmed/26056092) run using the [robumeta](https://cran.r-project.org/web/packages/robumeta/robumeta.pdf) package version 2.0. [Fisher & Tipton 2015](robumeta: an R package for robust variance estimation in meta-analysis) provide a guide for using the *robumeta* package. A detailed rationale for using the RVE approach is provided in our [study protocol](https://www.dropbox.com/s/5uqmbzfopolnckd/Protocol.docx?dl=1). 
...
* **minanycoinf**: Binary factor indicating whether or not the effect size estimate was based on an analysis that limited potential confounding due to the presence of co-infections
...
3. Load all required packages (note: you will have to install these packages first using the [install.packages()](https://www.rdocumentation.org/packages/utils/versions/3.6.1/topics/install.packages) function)
...
whoprimary <- read.csv( 'https://www.dropbox.com/s/ybre6b47l0jpj3z/whoprimary.csv?dl=1' )
...
First, we create an inverse standard error weight variable to show approximately how much weight each estimate would carry (i.e., how precise it is) for plotting (not analytic) purposes. The standard error for each log odds ratio effect size estimate is contained in the *se_logor* variable.
...
**Model 1.0:** Prior to running the *a priori* model, we first run a model with no effect modifiers to calculate the unconditional weighted mean log~e~OR. We use a simple working model of the within-study correlation matrix (set all within-study correlations to $\rho$ = 0.80) in order to get optimally efficient inverse variance weights. The **studyid** variable is identified as the clustering variable and small-sample-size corrections are applied [(Tipton, 2015)](https://www.ncbi.nlm.nih.gov/pubmed/24773356). The analysis provides traditional estimates of between-study heterogeniety ($\tau$^2^ and I^2^), but the RVE approach deemphasizes the evaluation of between-study heterogeneity and focuses only on obtaining accurate standard errors for fixed-effects parameters. $\tau$^2^ is calculated in RVE analyses only to allow for the calculation of optimally efficient weights [(Tanner-Smith et al. 2016)](https://link.springer.com/article/10.1007/s40865-016-0026-5). Therefore, although the *robumeta* package reports traditional between-study heterogeneity estimates, we do not focus on them. 
```{r }
...
* **x~5~^ij^** = neonatal health confounding: 0=no attempt to limit potential confounding due to infant neonatal health; 1= confounding limited at least somewhat
...
**Model 1.1:** We run the *a priori* model for *RSV-LRTI Exposure* studies. First, we set the reference level for all effect modifiers to their modal values. Consequently, the intercept represents the conditional mean effect size estimate when all effect modifiers are held at their most common value. 
```{r }
...
**Model 1.2:** We rerun Model 1.1 after collapsing the *school age* and *adolescence/adulthood* categories for the outcome age variable. It is now a binary variable called *preschool*. This effect modifier now compares effect size esitmates measured during the preschool years to estimates measured outside of the preschool years: preschool vs. schoolage/adolescence/adulthood. This may fix the *df* problem. Additionally, we dropped the two interaction terms as there was no evidence that they were significant and they had relatively small *df*.

```{r}
model1.2 <- robu( logor ~ surveilf + preschoolf + exp012f + minanygenf + minanycoinff + minanyneof + minanysmokef + participantriskf, 
                  data = whorsvexp, 
                  studynum = studyid, 
                  var.eff.size = se_logor^2, 
                  rho = .8, 
...
The conditional weighted mean OR = 2.69 (95% CI: 1.38, 5.27) when holding all effect modifiers at their modal levels.

Effect size estimates were larger among studies that did not limit genetic confounding: $\hat\gamma$=0.65, 95% CI [0.06, 1.24]. This is consistent with the hypothesis that RSV-LRTI exposure is at least partly a marker of genetic susceptibility rather than a strictly causal factor. There was no evidence of effect modification by whether estimates were based on analyses limiting the influence non-RSV co-infections or neonatal health markers. Although these effect modifiers did not represent primary hypotheses, estimates from studies using a targeted enrollment strategy (i.e., enrolling only those with a known risk factor for wheezing illness other than RSV-LRTI exposure: *participantrisk*) and those in which the exposure ascertainment period extended beyond the first 12 months of life (*exp012*) yielded smaller effect sizes. 


**Model 1.3:** To gauge how much the expected weighted mean effect size drops when limiting genetic confounding, we reran Model 1.2 changing the reference level of the *miananygenf* variable so that not limiting genetic confounding was the reference category and limiting genetic confounding at least somewhat was the comparison level. The intercept in this model represents the condition weighted mean effect size estimate among studies that did not limit genetic confounding, holding all other effect modifiers at their modal categories. 

```{r}
## Create a new limiting genetic confounding effect factor with "Not limited" as the reference level
whorsvexp$minanygenrev <- factor(whorsvexp$minanygen,levels=0:1,labels=c('Not limited','Limited'))
...
The intercept in Model 1.3 is considerably larger when it represents the weighted mean effect size among studies not limiting genetic confounding compared to Model 1.2 when it represents studies that did limit genetic confounding. The point estimate for the intercept increases from aOR=2.69 in Model 1.2 to aOR=4.26 in Model 1.3. The intercept is positive and signifcant in both models, indicating a positive conditional mean effect between RSV-LRTI and subsequent wheezing illness. But we expect a substantial drop in the OR for the effect of RSV-LRTI on subsequent wheezing illness when studies do something to limit genetic confounding. 


Below we visualize the modifying effect of limiting genetic confounding by plotting boxplots for the observared effect size estimates by whether or not the estimates limited genetic confounding. We superimpose the point estimates to show the observed distributions with the point size proportional to the estimate's inverse variance -- i.e., more precise estimates have larger points. Additionally, we superimpose red diamonds, the center of which represents the conditional mean effect size estimates based on models 1.2 and 1.3. The bottoms and tops of the diamonds represent, respectively, the lower and upper bound 95% confidence intervals. Finally, we highlight in blue the estimate from the study by [Poorisrisak et al. 2010](https://www.ncbi.nlm.nih.gov/pubmed/?term=poorisrisak+twin+rsv+asthma) because this was the only estimate that fully eliminated genetic confounding by comparing monozygotic twins who were discordant for RSV-LRTI hospitalization. This point estimate was smaller than over 80% of the estimates that only partly limited genetic confounding. As the sample size was relatively small in this study, there was considerable uncertainty in this estimate with plausible odds ratios ranging from RSV-LRTI hospitalization being protective (lower 95% CI = 0.36) to highly damaging (upper 95% CI = 4.00). 
```{r warning=F, fig.width = 10, fig.height = 8}
whorsvexp$minanygenplot <- factor( whorsvexp$minanygen, levels = 0:1, labels=c('Not limited', 'Limited at least somewhat') )
...
( box2 <- ggplot( whorsvexp, aes( x=minanygenplot, y=logor ) ) +
    ...
    geom_point( aes(size = whorsvexp$w, color = mingenfullf ), position = position_jitterdodge(),
                show.legend = F ) +
    scale_color_manual(values=c('Not fully minimized'='black', 'Fully minimized'='dodgerblue')) +
    ...
    annotate("text", x = 2.25, y =whorsvexp[ whorsvexp$studyname=='Poorisrisak 2010','logor']*.5, label = "Poorisrisak 2010", color='dodgerblue', fontface='bold') + 
    annotate("text", x = 1.32, y =model1.3$reg_table$b.r[1], label = paste( "aOR=",
                                                                            round(exp(model1.3$reg_table$b.r[1]), digits=2),
                                                                            " [", 
                                                                            round(exp(model1.3$reg_table$CI.L[1]), digits=2), 
                                                                            ", ",
                                                                            round(exp(model1.3$reg_table$CI.U[1]), digits=2),
                                                                            "]"), 
                                                                            color='firebrick', fontface='bold') + 
    annotate("text", x = 2.32, y =model1.2$reg_table$b.r[1], label = paste( "aOR=",
...
**Model 1.4:** We run a sensitivity analysis for Model 1.2 using different values for the presumed common correlation among effect sizes from the same study ($\rho$). We'll rerun the model 5 times with the value of $\rho$ taking on values ranging from 0.00 to 1.00 (full range of possible correlations) at an interval of 0.20. Then we can see if the model estimates and standard errors are sensitive to our choice of value for $\rho$.
...
This figure shows that, for all four primary parameters of interest, the removal of any one study (and all of its estimates) does not alter whether the 95% confidence interval contains the null value (log~e~OR=0). No individual study had sufficient leverage to alter our primary conclusions. In sum, the effects were robust and not driven by any one study. 

<br>

**Model 1.6:** Although not included in the *a priori* model for RSV-LRTI exposure studies, it is plausible that early second-hand smoking exposure could contribute to a posiitve spurious association between RSV-LRTI and subsequent wheezing illness. In this model, we add a binary effect modifier (*minanysmokef*) coding whether or not effect size estimates were based on analyses in which second-hand smoking exposure (either during pregnancy or in early life) to our primary RSV-LRTI exposure model (model 1.2). 

```{r}
## Set the reference level for the smoking exposure modifier to the model value (i.e., "not limited")
whorsvexp$minanysmokef <- relevel( whorsvexp$minanysmokef, ref = 'Not limited' )

model1.6 <- robu( logor ~ surveilf + preschoolf + exp012f + minanygenf + minanycoinff + minanyneof + minanysmokef + minanysmokef + participantriskf, 
                  data = whorsvexp, 
                  studynum = studyid, 
                  var.eff.size = se_logor^2, 
                  rho = .8, 
...
There is no evidence that limiting potential confounding due to early life 2nd-hand smoke exposure modifies effect sizes for the effect of RSV-LRTI exposure on subsequent wheezing illness. Further, adding this effect modifier does not alter our conclusions about any of the other primary effect modifiers of interest.
...
Next, we run Model 1.7. Because there are only 15 studies and 42 effect size estimates, we cannot reasonably run a model as complex as Model 1.2. Therefore, we first run the model without any effect modifiers. 
...
The weighted mean effect size is positive and significant: OR = 2.43, 95% CI [1.62, 3.63]. Thus, the effect of RSV-LRTI on subsequent wheezing illness does not appear to be driven by estimates evaluating effects of RSV-LRTI on transient wheezing episodes in early life. As shown below, there is no evidence that whether estimates were based on analyses limiting genetic confounding, co-infections, or neonatal health markers modified effect sizes. As the sample size was much smaller than in model 1.2, there was less power and we entered only one effect modifier at a time.

**Model 1.7a:** Rerun Model 1.7 including the variable coding whether or not the estimates were based on analyses limiting genetic confounding (*minanygenf*) as an effect modifier.

```{r}
( model1.7a <- robu( logor ~ minanygenf, 
                          data = whorsvexp_asth6, 
                          studynum = studyid, var.eff.size = se_logor^2, rho = .8, small = T ) )
```

**Model 1.7b:** Rerun Model 1.7 including the variable coding whether or not the estimates were based on analyses limiting non-RSV co-infections (*minanycoinf*) as an effect modifier.

```{r}
( model1.7b <- robu( logor ~ minanycoinf, 
                          data = whorsvexp_asth6, 
                          studynum = studyid, var.eff.size = se_logor^2, rho = .8, small = T ) )
```

**Model 1.7c:** Rerun Model 1.7 including the variable coding whether or not the estimates were based on analyses limiting differences in neonatal health markers (*minanyneo*) as an effect modifier.

```{r}
( model1.7c <- robu( logor ~ minanyneo, 
                          data = whorsvexp_asth6, 
                          studynum = studyid, var.eff.size = se_logor^2, rho = .8, small = T ) )
```
...
<br>
<br>
**Model 1.8:** We also evaluated whether the association between RSV-LRTI and subsequent wheezing illness was significant among the subset of estimates from samples in which all children had a family history of asthma or atopy.

```{r}
whoatopyexp <- subset( whoatopy, rsvexp == 'RSV Exposure Studies' )
( model1.8 <- robu( logor ~ 1, 
                          data = whoatopyexp, 
                          studynum = studyid, var.eff.size = se_logor^2, rho = .8, small = T ) )

model1.8.est <- round( exp( c(model1.8$reg_table$b.r[1]) ), digits = 2  )
model1.8.lci <- round( exp( c(model1.8$reg_table$CI.L[1]) ), digits = 2  )
model1.8.uci <- round( exp( c(model1.8$reg_table$CI.U[1]) ), digits = 2  )
## Estimates on the OR scale
( model1.8.table <- data.frame(parameter=model1.8$reg_table$labels[1], ConditionalOddsRatio=model1.8.est[1], LCI=model1.8.lci[1], UCI=model1.8.uci[1] ) )
```

The weighted mean log~e~(OR) effect size is greater than 0 in these 6 studies (40 estimates), but notably smaller than in the primary analyses: OR = 1.72, 95% CI[1.19, 2.49]. This raises the possibility that RSV-LRTI might have a smaller effect among children who have a high genetic loading for asthma/atopy, though our analyses could not test this explicitly. There were only 2 studies providing estimates from the specific subpopulation of children whose parents denied family history of asthma/atopy, precluding meaningful meta-analytic modeling.
...
In this section, we evaluate whether there is evidence that high-risk infants receiving RSV immunoprophylaxis have lower risk of subsequent wheezing illness. There are 8 *RSV immunoprophylaxis* studies (2 RCTs and 6 non-randomized studies) providing 24 estimates for the primary analyses. 

First, we create a data frame with only RSV immunoprophylaxis Studies. There are 16 estimates from non-randomized prophylaxis studies and 8 from the two RCTs. 
```{r}
...
whoprophyl$eslab <- c( 'Serious wheeze', ## O'Brien -.06
                       '3+ med.-attended wheeze', ## O'Brien -.09
                       '1+ medically-attended wheeze episodes', ## O'Brien 0.06
                       'Parent-reported asthma', ## Scheltema 0.66
                       'Physician dx asthma', ## Scheltema -0.04
                       'Wheeze ages 1-3', ## Scheltema 0.51
                       'Recurrent wheeze', ## Blanken 2013 0.74
                       'Any wheeze', ## Blanken 2013 0.69
...
text( labels = c( 'RCTs','Observational: Limit confounders','Observational: Naive etimates' ), 
      ...
      col=1) 
segments( x0 = c(-15,-15,-15), x1 = c(15,15,15), y0=c(51.5,37.5,12.5), col='dark gray', lwd = 2 )
...
with( prophylplot1, text( x=4.5, y = 1+.05, labels = round(model2.2$reg_table$b.r, digits=2), las = 2, font = 2, col = 'firebrick', cex = .9, xpd = T ) )
with( prophylplot1, text( x=6.5, y = 1, labels = paste( round(model2.2$reg_table$CI.L, digits=2), ", ", round( model2.2$reg_table$CI.U, digits = 2) ), 
                                                        las = 2, font = 2, col = 'firebrick', cex = .9, xpd = T  ) )
with( prophylplot1, text( x=4.5, y = 15+.055, labels = round(model2.3$reg_table$b.r, digits=2), las = 2, font = 2, col = 'dodgerblue', cex = .9, xpd = T  ) )
with( prophylplot1, text( x=6.5, y = 15, labels = paste( round(model2.3$reg_table$CI.L, digits=2), ", ", round( model2.3$reg_table$CI.U, digits = 2) ), 
...
As expected, the confidence interval is very large, spanning OR values from 0.04 to 38.95. So we cannot rule out that getting a placebo instead of RSV immunoprophylaxis has a protective effect or a highly damaging effect.
